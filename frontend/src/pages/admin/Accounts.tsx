  ...
  return (
    <div data-testid="admin-accounts">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-heading text-2xl font-black tracking-tight text-white">Accounts</h1>
          <p className="mt-1 text-sm text-slate-400">{users.length} organizer account{users.length === 1 ? "" : "s"} · each has their own login and access</p>
        </div>
            ...
            {users.map((u) => (
              <div key={u.id} className="rounded-md border border-slate-800 bg-obsidian p-3" data-testid={`account-card-${u.id}`}>
                <div className="flex items-start justify-between gap-3">
                  ...
                  <span className={u.is_active ? "rounded-md bg-emerald-500/15 border border-emerald-500/30 px-2 py-1 text-xs font-bold text-emerald-400" : "rounded-md bg-white/5 border border-white/10 px-2 py-1 text-xs font-bold text-slate-400"}>{u.is_active ? "Active" : "Deactivated"}</span>
                </div>
                ...
                <div className="mt-3 grid grid-cols-3 gap-2">
                  <Button variant="outline" size="sm" className="min-w-0 px-1 text-xs" onClick={() => toggleActive(u)}>{u.is_active ? "Disable" : "Enable"}</Button>
                  <Button variant="outline" size="sm" className="min-w-0 px-1 text-xs" onClick={() => openEdit(u)}>Edit</Button>
                  <Button variant="outline" size="sm" className="min-w-0 px-1 text-xs text-red-400 border-red-500/30" onClick={() => remove(u)}>Delete</Button>
                </div>
              </div>
            ))}
          </div>
            ...
            <tbody>
              {users.map((u, i) => (
                <TR key={u.id} data-testid={`account-row-${u.id}`}>
                  ...
                  <TD className="max-w-[14rem] text-xs text-slate-300">
                  ...
                  <TD className="max-w-xs text-xs text-slate-400">
                    {u.is_admin && <span className="mr-1.5 inline-flex items-center gap-1 rounded-md bg-coral/15 border border-coral/30 px-2 py-0.5 font-bold text-coral"><Crown className="h-3 w-3" /> Admin</span>}
                    {!u.is_admin && <span>{summarize(u)}</span>}
                  </TD>
                  <TD>
                    {u.is_active ? (
                      <span className="inline-flex items-center gap-1 rounded-md bg-emerald-500/15 border border-emerald-500/30 px-2 py-1 text-xs font-bold text-emerald-400">
                        <ShieldCheck className="h-3.5 w-3.5" /> Active
                      </span>
                    ) : (
                      <span className="inline-flex items-center gap-1 rounded-md bg-white/5 border border-white/10 px-2 py-1 text-xs font-bold text-slate-400">
                  ...
                  <TD><div className="flex justify-end gap-1">
                    ...
                    <Button variant="ghost" size="icon" onClick={() => remove(u)} data-testid={`delete-account-${u.id}`}><Trash2 className="h-4 w-4 text-red-400" /></Button>
                  </div></TD>
                </TR>
              ))}
            </tbody>
      ...
      <Dialog open={open} onClose={() => setOpen(false)} title={form.id ? "Edit Account" : "Add Account"} testId="account-dialog">
        <div className="space-y-4">
          ...
          <div>
            <div className="flex items-center justify-between">
              ...
              {form.staff_member_ids.length > 0 && (
                <button type="button" className="text-xs font-semibold text-slate-400 hover:text-white" onClick={() => setForm((f) => ({ ...f, staff_member_ids: [] }))}>Clear</button>
              )}
            </div>
            <p className="mt-0.5 text-xs text-slate-400">Every account here belongs to a real staff member — link who this login is for. One account can stand in for more than one person (e.g. a shared shift device).</p>
            <div className="mt-1.5 max-h-44 overflow-y-auto rounded-md border border-white/10 bg-white/5 p-2" data-testid="account-staff-list">
              {staff.map((s) => (
                <label key={s.id} className="flex items-center gap-2 rounded px-1.5 py-1 text-sm text-slate-200 hover:bg-white/10">
          ...
          <label className="flex items-center gap-2 rounded-md border border-white/10 bg-white/5 px-3 py-2.5 text-sm">
            ...
            <span className="text-xs text-slate-400">— full access to every section, plus managing accounts</span>
          </label>

          {!form.is_admin && (
            <div>
              <Label>Module Access</Label>
              <div className="mt-1.5 divide-y divide-white/10 rounded-md border border-white/10 bg-white/5">
