  ...
  return (
    <div data-testid="admin-procurement">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="font-heading text-2xl font-black tracking-tight text-white">Procurement</h1>
          <p className="mt-1 text-sm text-slate-400">{items.length} items tracked</p>
        </div>
            ...
            <tbody>
              {items.map((p, i) => (
                <TR key={p.id} data-testid={`procurement-row-${p.id}`}>
                  ...
                  <TD className="font-bold text-white">{p.title}{p.supplier && <div className="text-xs font-normal text-slate-400">{p.supplier}</div>}</TD>
                  ...
                  <TD className="text-right text-slate-300">{formatMoney(p.target_unit_price, p.currency)}</TD>
                  <TD className="text-right font-semibold text-white">{formatMoney(p.max_budget, p.currency)}</TD>
                  <TD className="text-slate-300">{p.owner || "—"}</TD>
                  <TD>
                    <div className="flex justify-end gap-1">
                      ...
                      <Button variant="ghost" size="icon" onClick={() => remove(p.id)} data-testid={`delete-procurement-${p.id}`}><Trash2 className="h-4 w-4 text-red-400" /></Button>
                    </div>
                  </TD>
                </TR>
              ))}
