...
export function PlaceholderPage({ title, section }: { title: string; section: string }) {
  return (
    <div className="mx-auto max-w-7xl px-5 md:px-8 py-20 md:py-24" data-testid={`placeholder-${section.toLowerCase()}`}>
      <span className="text-xs font-bold uppercase tracking-widest text-coral-600">{section}</span>
      <h1 className="mt-3 font-heading text-4xl font-black tracking-tight text-slate-950 sm:text-5xl">{title}</h1>
      <div className="mt-10 rounded-lg border border-dashed border-slate-300 bg-slate-50/70 p-10 md:p-14">
        <div className="grid h-12 w-12 place-items-center rounded-md bg-slate-900 text-white">
          <Construction className="h-6 w-6" />
        </div>
        <h2 className="mt-6 font-heading text-xl font-bold text-slate-900">This section is coming soon</h2>
        ...
        <p className="mt-4 inline-block rounded-md bg-white px-3 py-1.5 text-xs font-semibold text-slate-500 ring-1 ring-slate-200">
