...
export default function Home() {
  ...
  // Keyboard navigation for Host School Photos Lightbox modal (Escape, ArrowLeft, ArrowRight)
  useEffect(() => {
    if (schoolModal === null) return;
  ...
  // Mobile swipe gestures for host school modal
  ...
  const handleTouchEnd = (e: React.TouchEvent) => {
    ...
    if (Math.abs(diff) > 40 && schoolModal !== null) {
      const photos = getSchoolPhotos(schoolModal.category);
      if (photos.length > 1) {
        if (diff > 0) {
          setSchoolModal((prev) =>
