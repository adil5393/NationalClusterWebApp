...
import { Users, CalendarDays, MapPin, Megaphone, ArrowRight, BedDouble, Bus, UtensilsCrossed } from "lucide-react";
  ...
  return (
    <div data-testid="public-home">
      ...
      <section className="relative isolate overflow-hidden bg-obsidian text-white">
        <img src={HERO} alt="Host school campus" className="absolute inset-0 h-full w-full object-cover opacity-40" />
        ...
        <div className="relative mx-auto max-w-7xl px-5 md:px-8 pt-24 pb-28 md:pt-32 md:pb-36">
          <span className="inline-flex items-center rounded-md border border-white/20 bg-white/10 px-3 py-1 text-xs font-bold uppercase tracking-widest text-coral">
            National Cluster Event · Hosted by our School
          </span>
          <h1 className="mt-6 max-w-4xl font-heading text-4xl font-black leading-[1.05] tracking-tight sm:text-5xl lg:text-6xl">
            CLUSTER NATIONALS
          ...
          <div className="mt-9 flex flex-wrap gap-3">
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-md bg-coral px-6 py-3 text-sm font-bold text-white transition-colors hover:bg-coral-600"
            >
            ...
            <Link
              ...
              className="inline-flex items-center gap-2 rounded-md border border-white/25 bg-white/5 px-6 py-3 text-sm font-bold text-white transition-colors hover:bg-white/10"
            >
              View Announcements
            </Link>
          </div>
      ...
      <section className="border-b border-slate-200 bg-white">
        <div className="mx-auto grid max-w-7xl grid-cols-2 gap-px bg-slate-200 md:grid-cols-4">
          {[
            ...
            <div key={s.v} className="bg-white px-6 py-8">
              <p className="font-heading text-2xl font-black text-slate-950 md:text-3xl">{s.k}</p>
              <p className="mt-1 text-xs font-semibold uppercase tracking-wide text-slate-500">{s.v}</p>
            </div>
          ))}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-7xl px-5 md:px-8 py-20 md:py-24">
        <div className="max-w-2xl">
          <h2 className="font-heading text-3xl font-black tracking-tight text-slate-950">Everything for the event, in one place</h2>
        ...
        <div className="mt-12 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {FEATURES.map((f) => (
            <Link
              ...
              className="group rounded-lg border border-slate-200 bg-white p-6 transition-transform hover:-translate-y-1 hover:border-coral"
            >
              <div className="grid h-11 w-11 place-items-center rounded-md bg-slate-900 text-white transition-colors group-hover:bg-coral">
                <f.icon className="h-5 w-5" />
              </div>
              <h3 className="mt-5 font-heading text-lg font-bold text-slate-950">{f.title}</h3>
              <p className="mt-1.5 text-sm leading-relaxed text-slate-600">{f.desc}</p>
              <span className="mt-4 inline-flex items-center gap-1 text-sm font-bold text-coral-600">
      ...
      <section className="relative isolate overflow-hidden bg-slate-900 text-white">
        <img src={CROWD} alt="Competition crowd" className="absolute inset-0 h-full w-full object-cover opacity-25" />
        <div className="relative mx-auto max-w-7xl px-5 md:px-8 py-20 text-center md:py-24">
          ...
          <p className="mx-auto mt-4 max-w-xl text-slate-300">
            Team-specific portals with accommodation, transport and schedule details are coming soon. For now, browse
            the participating teams.
          </p>
          <Link
            to="/teams"
            className="mt-8 inline-flex items-center gap-2 rounded-md bg-coral px-6 py-3 text-sm font-bold text-white transition-colors hover:bg-coral-600"
          >
