        ...
        <div className="mx-auto max-w-7xl px-5 md:px-8">
          <div className="flex h-16 items-center justify-between gap-4">
            <Link to="/" className="flex items-center gap-2.5 shrink-0" data-testid="brand-logo">
              <span className="grid h-9 w-9 place-items-center rounded-md bg-coral font-heading text-sm font-black text-white">
                CN
              </span>
              <span className="hidden sm:block">
                <span className="block font-heading text-sm font-extrabold leading-none tracking-tight text-white">
                  CLUSTER NATIONALS
                </span>
                <span className="block text-[11px] font-semibold tracking-widest text-coral">2026–27</span>
              </span>
            </Link>
            ...
            <div className="flex items-center gap-2">
              <Link
                ...
                className="hidden sm:inline-flex items-center gap-1.5 rounded-md bg-coral px-3.5 py-2 text-[13px] font-bold text-white transition-colors hover:bg-coral-600 shadow-sm"
              >
                Organizer Portal <ArrowUpRight className="h-3.5 w-3.5" />
              </Link>
              <button
                className="xl:hidden grid h-10 w-10 place-items-center rounded-md border border-white/15 bg-white/5 text-slate-200 hover:bg-white/10"
                onClick={() => setOpen((v) => !v)}
                ...
                <NavLink
                  ...
                  className={({ isActive }) =>
                    cn(
                      ...
                      isActive ? "bg-coral text-white" : "text-slate-300 hover:bg-white/10",
                    )
                  }
                >
                  {n.label}
      ...
      <footer className="border-t border-white/10 bg-black/60 text-white">
        <div className="mx-auto max-w-7xl px-5 md:px-8 py-14">
          <div className="grid gap-8 md:grid-cols-3">
            <div>
              <span className="font-heading text-lg font-black tracking-tight text-white">CLUSTER NATIONALS 2026–27</span>
