...
export function PublicLayout() {
  ...
  return (
    <div className="relative min-h-screen bg-obsidian-950 text-slate-100 flex flex-col selection:bg-gold selection:text-obsidian overflow-x-hidden">
      {/* 1. FIXED BACKGROUND SLIDESHOW LAYER (CONSTRAINED TO MAX-W-7XL CONTENT CONTAINER) */}
      <div
        className="fixed inset-y-0 left-1/2 -translate-x-1/2 w-full max-w-7xl pointer-events-none overflow-hidden select-none z-0 border-x border-white/5"
        aria-hidden="true"
      >
        ...
        <div className="border-b border-white/10 bg-obsidian-950/80 backdrop-blur-md px-3 sm:px-4 md:px-6 lg:px-8 py-1.5 text-xs text-slate-400">
          <div className="mx-auto flex max-w-7xl 2xl:max-w-[1600px] items-center justify-between gap-3 sm:gap-4">
            <div className="flex items-center gap-2 overflow-hidden min-w-0 flex-1">
              ...
              <span className="truncate text-[11px] text-slate-400 hidden sm:inline">
                CBSE National Kabaddi Championship 2026–27 • Host: New Angels Sr. Sec. School, Pratapgarh
              </span>
            </div>
            <div className="hidden md:flex items-center gap-3 lg:gap-4 text-[11px] font-medium shrink-0 whitespace-nowrap">
              <span className="flex items-center gap-1.5 text-emerald-400">
                <span className="h-2 w-2 rounded-full bg-emerald-400 animate-live-dot shrink-0" />
        ...
        <header className="sticky top-0 z-40 border-b border-white/10 bg-obsidian-950/90 backdrop-blur-xl transition-colors">
          <div className="mx-auto max-w-7xl 2xl:max-w-[1600px] px-3 sm:px-4 md:px-6 lg:px-8">
            <div className="flex h-16 items-center justify-between gap-2 sm:gap-4">
              ...
              >
                <div className="relative grid h-9 w-9 sm:h-10 sm:w-10 place-items-center rounded-lg border border-gold/40 bg-white/[0.06] p-1 shadow-[0_0_15px_-3px_rgba(245,158,11,0.25)] transition-transform group-hover:scale-105 shrink-0">
                  <img
                    src={schoolLogo}
                ...
                <div className="leading-tight min-w-0">
                  <div className="flex items-center gap-1 sm:gap-1.5">
                    <span className="font-heading text-xs sm:text-sm md:text-base font-black tracking-tight text-white group-hover:text-gold transition-colors whitespace-nowrap truncate">
                      NATIONAL KABADDI
                    </span>
                    <span className="rounded bg-gold/15 px-1 sm:px-1.5 py-0.5 text-[8px] sm:text-[9px] font-black tracking-wider text-gold shrink-0">
                      CBSE
                    </span>
                  </div>
                  <span className="block text-[9.5px] sm:text-[10px] md:text-[11px] font-heading font-extrabold tracking-wider sm:tracking-widest text-slate-400 whitespace-nowrap truncate">
              ...
              >
                {NAV.map((n) => (
                  ...
                  >
                    ...
                    <span>{n.label}</span>
                    {n.isLive && (
                      <span className="rounded bg-emerald-500/20 px-1.5 py-0.5 text-[9px] font-black tracking-wider text-emerald-300 shrink-0">
                        LIVE
                      </span>
                    )}
                  </NavLink>
              ...
              <div className="flex items-center gap-2 sm:gap-2.5 shrink-0">
                <Link
                  ...
                  className="hidden sm:inline-flex items-center gap-1.5 rounded-md border border-coral/40 bg-coral px-3 sm:px-3.5 py-1.5 sm:py-2 text-xs font-heading font-extrabold tracking-wide text-white transition-all hover:bg-coral-600 hover:shadow-lg shadow-sm active:scale-95 whitespace-nowrap shrink-0"
                >
              ...
              >
                <div className="mb-3 flex items-center justify-between px-2">
                  ...
                  <Link
                    ...
                    className="inline-flex sm:hidden items-center gap-1 rounded bg-coral/20 border border-coral/30 px-2.5 py-1 text-xs font-heading font-bold text-coral hover:bg-coral/30 transition-colors"
                  >
                    <span>Organizer Portal</span>
                ...
                <nav className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-2">
                  {NAV.map((n) => (
                    <NavLink
                      ...
                      className={({ isActive }) =>
                        cn(
                          "flex items-center justify-between rounded-lg border px-3 py-2.5 text-xs font-heading font-bold transition-colors whitespace-nowrap",
                          isActive
